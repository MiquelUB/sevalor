...
from app.models.models import TokenInvitacioTelegram, Client
...
class TelegramService:
    ...
    async def send_message(self, chat_id: int, text: str) -> bool:
        if not self.bot_token or self.bot_token == "DUMMY_TOKEN":
            logger.info(f"[TELEGRAM SIMULAT] Missatge a {chat_id}: {text}")
            return True
    ...
    async def processar_comanda_start(self, db: AsyncSession, chat_id: int, payload_text: str) -> str:
        ...
        stmt = select(TokenInvitacioTelegram).where(TokenInvitacioTelegram.token_hash == token_str, TokenInvitacioTelegram.utilitzat == False)
    ...
    async def processar_missatge_general(self, db: AsyncSession, chat_id: int, text: str) -> str:
        from app.models.models import FaqCorporativaRag, Empresa
        ...
        context_rag = ""
        if filtres_rag:
            q_faq = select(FaqCorporativaRag).where(FaqCorporativaRag.empresa_id == empresa_id, FaqCorporativaRag.actiu == True).where(or_(*filtres_rag))
