...
test.describe.serial('Auditoria de Test 1: PWA Login E2E', () => {
  ...
  test('Test 1: Login reeixit injectant JWT al localStorage', async ({ page }) => {
    ...
    await nifInput.fill('12345678A'); // Operari creat al seed_db.py
    ...
    await page.waitForFunction(() => localStorage.getItem('sevalor_auth_token') !== null || document.cookie.includes('sevalor_auth_token'), null, { timeout: 10000 });

    const token = await page.evaluate(() => localStorage.getItem('sevalor_auth_token'));
